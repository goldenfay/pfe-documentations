...
import argparse,time,os,sys,datetime
import configparser
import csv
import requests
...
class YOLO(DetectionModel):
    ...
    def __init__(self, model_name):
        super(DetectionModel,self).__init__(model_name)
        self.net=YOLO.load_net()
        self.init_params()

    @classmethod
    def load_net(cls):
        cls.config = read_config('config.ini')

        # Load the trained network
        (net, cls.ln, cls.LABELS) = load_network('yolo-coco',tiny_version=True)
        return net

    def forward(self,frame):
         # Feed frame to network
        layerOutputs = forward_detection(frame, self.net, self.ln)
        # Obtain detected objects, including cof levels and bounding boxes
    ...
    def init_params(self):
        slef.confidence = float(self.config['NETWORK']['Confidence'])
        slef.threshold = float(self.config['NETWORK']['Threshold'])
