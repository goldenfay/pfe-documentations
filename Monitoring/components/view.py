...
def numpy_to_pil(array):
    ...
    return Image.fromarray(np.uint8(cm.jet(array)*255))


def numpy_to_b64(array):
    im_pil = numpy_to_pil(array)
    buff = _BytesIO()
                                    ...
                                    children=[
                                        ...
                                        html.Div(
                                            ...
                                            children=[
                                                ...
                                                html.Div(
                                                    [
                                                        # dbc.DropdownMenu(className="w-100 ",caret=True,
                                                        #                  bs_size="lg",
                                                        #                  children=[
                                                        ...
                                                        #                          divider=True),
                                                        #                      dbc.DropdownMenuItem(
                                                        #                          "Active and disabled", header=True),
                                                        #                      dbc.DropdownMenuItem(
        ...
        def update_output(list_of_names, list_of_contents):
            if list_of_contents is not None:
                ...
                children += [html.Div(children=[html.Button(
                    ...
                    children=[html.Span('Process Image(s)'), html.Span(className='fa fa-angle-right')])],
                    style={'font-weight': 'bold',
                           'font-size': '13px'}
                ),
                    html.Div(id='output-image-process', children=[''])
                ]
        ...
        def start_detection(button_click, model_type, children):
            if button_click > 0:
                ...
                for id, frame in enumerate(frames):
                    print('frame of shape : ', frame.shape)
                    try:
