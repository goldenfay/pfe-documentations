...
class Model(NN.Module):
    ...
    def __init__(self):
        ...
        self.checkpoints_dir = os.path.join(
            utils.BASE_PATH, 'checkpoints2', self.__class__.__name__)
        self.git_manager = GitManager(
            user='ihasel2020@gmail.com', pwd='pfemaster2020')
        ...
        with torch.no_grad():
            for i, (img, gt_dmap) in enumerate(test_dataloader):
                ...
                if i % 10 == 0:
                    ...
                    est_dmap = est_dmap.squeeze().cpu().permute(1,2,0).numpy()
                    ...
                    gt_dmap=gt_dmap.squeeze().cpu().permute(1,2,0).numpy()
